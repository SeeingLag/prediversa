...
try:
    ...
    cursor = condb.cursor()
    cursor.execute("SELECT * FROM master.dbo.Usuarios")  
    row = cursor.fetchone()
    print(row)
...
def login():
    if request.method == 'POST':
        perfil_form = request.form['perfil']
        usuario = request.form['usuario']
        contrasena = request.form['contrasena']

        condb = obtener_conexion()
        cursor = condb.cursor()

        cursor.execute("SELECT perfil FROM Usuarios WHERE usuario=? AND contrasena=?", (usuario, contrasena))
        usuario_db = cursor.fetchone()
        
        condb.close()

        if usuario_db: 
            perfil_db = usuario_db[0]  

            if perfil_form == perfil_db:  
                session['usuario'] = usuario
                session['perfil'] = perfil_db

                
                if perfil_db == "Administrador":
                    return redirect(url_for('administrador'))
                elif perfil_db == "Moderador":
                    return redirect(url_for('dashboard'))
                elif perfil_db == "Usuario":
                    return redirect(url_for('listar'))
            else:
                mensaje = f"⚠️ El perfil seleccionado no coincide con el registrado."
...
def administrador():
    if 'usuario' in session:
        ...
        return render_template('administradores.html', usuario=usuario, perfil=perfil)
    else:
...
def dashboard():
    if 'usuario' in session:
        usuario = session['usuario']
        perfil = session['perfil']
        return render_template('dashboard.html', usuario=usuario, perfil=perfil)
    else:
        return redirect(url_for('login'))
...
def listar():
    if 'usuario' in session:
        usuario = session['usuario']
        perfil = session['perfil']
        return render_template('cuestionarios/listar.html', usuario=usuario, perfil=perfil)
    else:
        return redirect(url_for('login'))
...
@app.route('/listar_cuestionarios')
def listar_cuestionarios():
    return "Lista de cuestionarios disponibles"
